...
function uploadImage($file, $upload_dir, $thumb = null, $id = null)
{
	...
	// $filename = ucfirst($upload_dir) . '-' . date('Ymdhis') . rand(0, 99) . '_' . $id . '.' . $file->getClientOriginalExtension();
	$filename = ucfirst($upload_dir) . '-' . date('Ymdhis') . rand(0, 99) . '_' . $id . '.jpg';
...
class UserInfo
{
	...
	public static function get_os()
	{
		...
		$os_array       =   array(
			'/windows nt 10/i'     	=>  'Windows 10 or 11',
...
use PIL\Image;
use App\Feature\FeatureExtractor;

class Train
{
	public function extract($folder_path)
	{
		$fe = new FeatureExtractor();

		$files = Storage::files($folder_path);
		foreach ($files as $file) {
			...
			$feature_path = 'public/features/' . pathinfo($file, PATHINFO_FILENAME) . '.npy';
			Storage::put($feature_path, serialize($feature));
		}
	}
}
