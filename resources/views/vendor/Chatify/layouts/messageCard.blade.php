        ...
        @if (@$attachment->type != 'image' || $message)
            <div class="message">
                @if (auth()->user()->role == 'customer')
                    @php
                        // Extract the Product ID number using regular expressions
                        // Remove the Product ID line and extract the Product ID number using regular expressions
                        if (preg_match("/Product ID: (\d+)\n/", $message, $matches)) {
                            $product_id = $matches[1];
                            $message = preg_replace("/Product ID: \d+\n/", '', $message);
                        } else {
                            $product_id = null;
                        }
                        // $user_id = \App\Models\Product::where('id', intval($product_id))->value('vendor_id');
